...
pub async fn get_communities(
    ...
    let role_map: std::collections::HashMap<&str, &str> = memberships
        .iter()
        .map(|m| (m.community_id.as_str(), m.role.as_str()))
        .collect();

    let _current_name = summaries
        .iter()
    ...
    let body = format!(
        ...
        header =
            render::header_with_switcher(i18n::JA_NAV_COMMUNITIES, community_id, &community_pairs),
        rows = rows,
