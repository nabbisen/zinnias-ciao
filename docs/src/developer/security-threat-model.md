...
| Missing, invalid, or silently replaced HMAC pepper | One secret-only resolver validates `HMAC_PEPPER`; every dynamic request preflights it before authentication, D1/KV use, or handler dispatch. Failure returns a fixed non-mutating `503`, while `/healthz` reports not ready. Local setup preserves an existing valid key and hosted replacement requires an explicit destructive-rotation flow. | RFC-077, `crypto.rs`, route-classifier/native tests, contracts source gates, and isolated local configuration/runtime gates. | Deployment-time rejection, runtime-negative, and exact-candidate valid-secret evidence remain hosted RFC-050 work; local implementation does not close B2. |
