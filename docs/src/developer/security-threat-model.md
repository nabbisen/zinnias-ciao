...
| Missing, invalid, or silently replaced HMAC pepper | One secret-only resolver validates `HMAC_PEPPER`; every dynamic request preflights it before authentication, D1/KV use, or handler dispatch. Failure returns a fixed non-mutating `503`, while `/healthz` reports not ready. Local setup preserves an existing valid key and hosted replacement requires an explicit destructive-rotation flow. | RFC-077, `crypto.rs`, route-classifier/native tests, contracts source gates, isolated local configuration/runtime gates, and architecture-accepted exact-candidate hosted required-secret/runtime-negative/valid-secret/deletion evidence. | B2 is closed. Operators must still attach the intended secret to each real target and avoid unintended rotation; unrelated RFC-050, production, public-pilot, and launch evidence remains open. |
