...
- [x] Form token absent/invalid → POST rejected; replay follows each workflow's reviewed idempotent branch. `form_token.rs::consume_detailed` distinguishes `Proceed` from every `Replay`, including `Replay(None)`, after a conditional UPDATE. The compatibility wrapper preserves the optional result reference but must not be used with `Option::is_some` to classify replay. Subject is the authenticated `user_id`. *(codlet.rs, form_token.rs, admin/members.rs — RFC-037, RFC-076)*
- [x] Admin invite generation reveals plaintext once only in the direct `200` body at a clean URL with `Cache-Control: no-store, private` and `Referrer-Policy: no-referrer`; token replay creates nothing and redirects cleanly; required-audit failure returns generic `503` and rolls back the invite. Legacy `code` query keys redirect before authentication/binding access. *(RFC-076 native/contracts gates and local `smoke:invite`; hosted RFC-050 and manual no-JS evidence remain open.)*
...
- [x] Class A construction/storage/cardinality failures centrally emit exactly one bounded `audit.required_batch_failed`; compiled-SSR route proofs cover invite generation, community creation primary-action ownership, join assertion rollback, and a post-trigger success control on disposable local D1. *(local correction; architecture implementation review pending)*
- [~] Packages 0A–8 are locally reviewed, but the Class A telemetry correction's implementation review and RFC-050 exact-candidate hosted rollback, disclosure, concurrency, migration, and teardown evidence remain open.
