...
- [x] Form token absent/invalid → POST rejected; replay follows each workflow's reviewed idempotent branch. `form_token.rs::consume_detailed` distinguishes `Proceed` from every `Replay`, including `Replay(None)`, after a conditional UPDATE. The RFC-076 generation path therefore uses the detailed result rather than `Option::is_some`; the compatibility wrapper still preserves an optional result reference for older workflow-specific handling. Subject is the authenticated `user_id`. *(codlet.rs, form_token.rs, admin/members.rs — RFC-037, RFC-076)*
