...
active theme** as of 2026-08-15, with **RFC-083** accepted 2026-08-16 to discharge
the localization Slice D that RFC-072 deferred — sequenced *after* RFC-054 slice 1.
...
| 083 | [Localization Slice D: Admin, Anonymous, and Unresolvable Surfaces](./accepted/083-localization-slice-d-admin-anonymous-and-unresolvable-surfaces.md) | 2026-08-16 | Discharges the Slice D that **RFC-072** deferred as a future RFC. Measured: **409** bare `i18n::JA_*` render sites over **224** constants — but **216 already have an English half**, so this is plumbing, not translation. **Slice D1 (admin)** is the bulk and costs no extra D1 query: `require_admin` already returns a `MembershipContext` carrying a resolved locale that handlers discard. **Slice D2 (anonymous routes) is blocked on §8** — whether `/join`/`/relink`/recovery should honour `Accept-Language`, untrusted input on the redemption path; recommendation is to stay Japanese pending Stage 0 research. **D1 must not start before RFC-054 slice 1 lands** (§7), or changed wording and changed plumbing become indistinguishable in one diff |
