# RFC 025 — Community Moderation, Abuse Response, and Member Safety
...
This RFC defines moderation and abuse response for small communities. The product should not be a social network, but comments and membership actions still need safe handling.
...
- Support admin deletion/hiding of inappropriate notes.
- Support member removal and re-add policy.
- Record moderation actions without exposing unnecessary details.
- Keep language calm and non-punitive for ordinary mistakes.
...
- No complex tribunal/workflow system.
...
|---|---|
| Delete note | Admin removes a harmful note with confirmation. |
| Remove member | Admin removes member access while preserving historical event records as configured. |
| Member sees removal | Removed member no longer sees community data. |
| Audit review | Admin can review moderation actions in a simple history view. |
...
Moderation should prefer soft actions. Notes can be marked `hidden_by_admin_at` while retaining an audit record. Member removal sets `removed_at` on membership rather than deleting the user. Historical participation may remain visible as “Former member” depending on privacy policy. The audit log should store actor, action type, target kind, target ID, community ID, timestamp, and short reason code.
...
- Never expose deleted harmful text broadly after removal.
- Avoid leaking removed member data to new members unless necessary.
- All moderation actions must be authorization-checked and audited.
- Hard deletion should be rare and operationally controlled.
